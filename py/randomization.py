...
@jit(nopython=True)
def _targeting_rewiring_d_v_two_(H_E_size: np.ndarray, H_jnt_node_deg: np.ndarray,
...
def _setup_for_d_v_two_five(H: hypergraph.HyperGraph, randH: hypergraph.HyperGraph):
    ...
    if B_M != randB_M:
        print(
            "ERROR: numbers of pairs between nodes and hyperedges are not the same between the given hypergraph and randomized one.")
        return
    ...
    return (
        H_degree_node_redun_coeff, randH_E, randH_E_index, randH_E_size, randH_elist, randH_elist_index, randH_node_deg,
...
def _targeting_rewiring_d_v_two_five(H_degree_node_redun_coeff: np.ndarray,
    ...
    included = np.zeros(N, dtype=types.int8)
    ...
    for r in range(0, R):
        ...
        for i_ in range(0, randH_node_deg[v_i]):
            ...
            for j_ in range(0, randH_E_size[e_i]):
                ...
                if included[w] == 0 and w != v_i and w != -1:
                    s3.append(w)
                    included[w] = 1
        ...
        for i_ in range(0, randH_node_deg[v_j]):
            ...
            for j_ in range(0, randH_E_size[e_j]):
                ...
                if included[w] == 0 and w != v_j and w != -1:
                    s3.append(w)
                    included[w] = 1
